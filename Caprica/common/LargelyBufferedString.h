...
struct LargelyBufferedString {
  ...
  LargelyBufferedString(std::string_view view) { this->append(view); }

  LargelyBufferedString& append(std::string_view view) {
    if (oolBuf != nullptr) {
      oolBuf->append(view);
    } else if (view.size() + bufLen >= sizeof(buf)) {
  ...
  void push_back(char c) { this->append(std::string_view(&c, 1)); }

  std::string_view string_view() { return std::string_view(data(), size()); }
