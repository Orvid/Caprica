...
int32_t PexInstruction::getDestArgIndexForOpCode(PexOpCode op) {
  switch (op) {
    ...
    case PexOpCode::TryLockGuards:
      return 0;
#define OP_ARG1(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
#define OP_ARG2(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
#define OP_ARG3(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
#define OP_ARG4(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
#define OP_ARG5(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
#define OP_ARG6(name, opcode, destArgIdx, ...) \
  case PexOpCode::opcode:                      \
    return destArgIdx;
      OPCODES(OP_ARG1, OP_ARG2, OP_ARG3, OP_ARG4, OP_ARG5, OP_ARG6)
#undef OP_ARG1
